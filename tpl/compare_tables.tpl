...
FROM (
	...
	WHERE src.database_name = '{{.Src}}' AND dst.column_type <> src.column_type
		AND (
			( CHARINDEX('CHAR',dst.column_type,0) > 0 AND CHARINDEX('CHAR',src.column_type,0) > 0 AND dst.column_length < src.column_length )
				OR
			( dst.column_type = 'TINYINT' AND src.column_type = 'INT' )
				OR
			( dst.column_type = 'DATE' AND src.column_type = 'DATETIME')
		)
	UNION ALL 
	/* [9] 非安全修改字段  */
	...
	WHERE src.database_name = '{{.Src}}' AND dst.column_type <> src.column_type
		AND NOT (
			  ( CHARINDEX('CHAR',dst.column_type,0) > 0 AND CHARINDEX('CHAR',src.column_type,0) > 0 )
				OR
			  ( CHARINDEX('INT',dst.column_type,0) > 0 AND CHARINDEX('INT',src.column_type,0) > 0 )
				OR
			  ( CHARINDEX('DATE',dst.column_type,0) > 0 AND CHARINDEX('DATE',src.column_type,0) > 0 )
		)
) TT
